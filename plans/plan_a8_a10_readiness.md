...
| A8-4 | OCR accuracy + provider comparison bench | **ACR-36** | ✅ done — `backend/scripts/ocr_bench/`, 7-layout labelled corpus, fuzzy line-item scorer, asserted no-regression gate; retires KI-09's anecdotal claim with a measured one; plan at `plans/plan_ticket_36.md` | `ticket-36/ocr-accuracy-bench` |
| A8-5 | Comparative concurrency study (3-arm ablation) | **ACR-44** | ✅ done — 4-arm ablation, 2→32 closers; ADR-02 the only arm holding 100% success and 0 lost updates at every level | `ticket-44/comparative-concurrency-study` (#38, #39) |
...
| **A8-4** | OCR accuracy + provider comparison bench (ACR-36) | 2026-07-31 | `ticket-36/ocr-accuracy-bench` | `backend/scripts/ocr_bench/` — deterministic Pillow-rendered corpus (7 labelled layouts: gridded, borderless_cramped, rotated, multipage, spanish, poor_scan, **degraded_fax**), fuzzy line-item alignment (`difflib.SequenceMatcher`, 0.75 threshold) fixing the positional-alignment defect where one dropped row shifted every later comparison. Fixed two defects on the way: `confidence` was a **fill rate** not accuracy — kept for the 422/fallback trigger it already gates, supplemented with an honestly-named `header_fill_rate` and a `provider` field that was computed and thrown away. **Retires KI-09's anecdotal claim with a measured one:** the 7-layout corpus shows `borderless_cramped` scoring the same as `gridded` on both providers — layout alone no longer degrades extraction — but `degraded_fax` (downscaled/grainy/skewed) produces genuine misreads on both (a BOL reference and an item name both hallucinated). **Baseline** (`tests/fixtures/ocr/baseline.json`, 3 rounds × 7 docs, 0.05 tolerance): gemini-2.5-flash header 96.25% / item-F1 99.25%, claude header 96.43% / item-F1 97.1%. **Caught a real quota bug measuring it:** `gemini-2.5-pro` returns `429 RESOURCE_EXHAUSTED` in ~142ms on every call on this API key — flash is the default, pro is one env var away for when quota exists. Asserted no-regression gate (`test_ocr_accuracy.py`, `OCR_BENCH_LIVE=1`) with a negative control in both directions — a corrupted extraction must score below the gate, *and* a metric the baseline enforces but a run doesn't report must fail rather than silently pass. **Caveat stated, not buried:** synthetic BOLs are cleaner than a real phone photo, so these figures are an upper bound; real-corpus validation stays open under RSK-03. `validation-run.sh` stage 6c → 6e (renumbered on merge to make room for A8-2/A8-5's own new stages). **Follow-up (#41) closed two can't-fail defects:** the script printed a failed gate but exited 0 regardless, and the date scorer guessed at ambiguous DD/MM vs MM/DD dates instead of refusing them — both fixed, so the gate now enforces what it claims. |
| **A10-1 + A10-4** | Containerize the stack & pin runtime versions (ACR-42) | 2026-07-31 | `ticket-42/docker-stack-versions` (#37) | `backend/Dockerfile` + `frontend/Dockerfile`; compose now `db` → `migrate` → `backend` → `frontend` (+ `seed` profile). One Node (24) and one Python (3.13) pinned repo-wide; `backend/requirements.lock` locks the transitive closure. |
| **A8-5** | Comparative concurrency study (ACR-44) | 2026-07-31 | `ticket-44/comparative-concurrency-study` (#38, #39) | `scripts/validation/concurrency_bench.py` — 4-arm ablation over one workload, 2→32 closers; ADR-02's protocol is the only arm holding 100% success and 0 lost updates at every level (goodput 95→227 closes/s). Follow-up (#39) added a two-directional correctness oracle, a contention witness, and a flake fix. |
