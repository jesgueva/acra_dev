...
**Date:** 2026-07-30
**§8 board last reconciled against Linear + GitHub:** 2026-08-01, `master` @ `441fc07`
(`fix(ACR-36): unbreak master — drift guard asserted cross-host byte identity (#49)`). ACR-45
(`ticket-45/aggregation-benchmark`, [PR #44](https://github.com/jesgueva/acra_dev/pull/44)) is open
and mergeable, with **Backend Tests** currently red on the same cross-host PNG byte-identity flake
`#49` fixed on master — the branch predates that fix and needs a rebase, not a code change. Treated
as done below per direction: the migration is built and measured, and the PR is otherwise complete.
...
| A8-6 | Aggregation benchmark at volume (RSK-04) | **ACR-45** | ✅ done, PR open — migration `015` adds `(product_id, status) INCLUDE (quantity_on_hand)`, resolving RSK-04: unindexed on-hand aggregation cost grew with table size (2.7 ms → 13.1 ms, 1k→200k lots) while indexed cost stayed flat (0.036 ms → 0.046 ms), a 75×–285× speedup as `Seq Scan` → `Index Only Scan`; the periodic-snapshot fallback RSK-04 also proposed is retired as unnecessary. Opens **RSK-08**: `export_csv` is unpaginated/unstreamed, 947 ms p50 / 4.3 s p95 at 200k lots — the slowest path in the system, deliberately not fixed here. pytest 533 passed/96%, Jest 146/146, Playwright 100/100. **Backend Tests check is currently red** on the same cross-host PNG byte-identity flake `#49` fixed on master — the branch predates that fix, not a defect in the aggregation work; needs a rebase before merge. Own §8.2 evidence row lands with the merge, per the pattern A8-4 already set | [PR #44](https://github.com/jesgueva/acra_dev/pull/44) (open) — `ticket-45/aggregation-benchmark` |
