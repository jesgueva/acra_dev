...
**Date:** 2026-07-30
**§8 board last reconciled against Linear + GitHub:** 2026-08-01, `master` @ `0fa3a59`
(`ticket-51: A10-7 — README troubleshooting, expected-output transcript, runbook (#55)`). Every A8
and A10 work item is now merged — the only thing still open is the §2.2 ledger decision gate,
which is a decision, not a work item.
...
| A8-2 | Benchmark harness — run metadata, p50/p95/p99, machine-readable output | **ACR-43** | ✅ done — `app/core/benchmark.py` (percentiles + `RunMetadata` provenance); feeds `api_latency_bench.py` at `validation-run.sh` stage 6c. | `d2e9520` ([PR #36](https://github.com/jesgueva/acra_dev/pull/36)) |
...
| A8-4 | OCR accuracy + provider comparison bench | **ACR-36** | ✅ done — 7-layout labelled corpus, fuzzy line-item scorer, asserted no-regression gate. Three review-loop follow-ups (#41, #47, #49) fixed a swallowed exit code, converged on the shared percentile convention, and closed a cross-host drift bug. | `8b84f9a` ([PR #40](https://github.com/jesgueva/acra_dev/pull/40)) + [#41](https://github.com/jesgueva/acra_dev/pull/41) + [#47](https://github.com/jesgueva/acra_dev/pull/47) + [#49](https://github.com/jesgueva/acra_dev/pull/49) |
| A8-5 | Comparative concurrency study (3-arm ablation) | **ACR-44** | ✅ done — 4-arm sweep; ADR-02 is the only arm holding 100% success and 0 lost updates at 2→32. Two follow-ups (#39, #45) hardened the correctness oracle and fixed a SERIALIZABLE misclassification. | `0547b38` ([PR #38](https://github.com/jesgueva/acra_dev/pull/38)) + [#39](https://github.com/jesgueva/acra_dev/pull/39) + [#45](https://github.com/jesgueva/acra_dev/pull/45) |
| A8-6 | Aggregation benchmark at volume (RSK-04) | **ACR-45** | ✅ done — migration `015` indexes `inventory_lots`; aggregate reads go 14.1 ms → 0.029 ms at 200k lots (487×), resolving RSK-04. Opened new risk RSK-10 (`export_csv` unpaginated). | `fbf1703` ([PR #44](https://github.com/jesgueva/acra_dev/pull/44)) |
| A8-7 | Subsystem diagram marking measurement points | **ACR-49** | ✅ done — `docs/architecture.md` maps `validation-run.sh`'s six stages onto the component boundaries they cross. Docs-only. | `ea5dec5` ([PR #52](https://github.com/jesgueva/acra_dev/pull/52)) |
| A10-1 | Dockerfiles for backend + frontend; whole-stack compose | **ACR-42** | ✅ done — new Dockerfiles; compose now runs `db → migrate → backend → frontend` (+ `seed` profile). | `098cc2e` ([PR #37](https://github.com/jesgueva/acra_dev/pull/37)) |
| A10-2 | OCR offline/mock mode + committed sample fixtures | **ACR-50** | ✅ done — `OCR_MOCK_MODE` returns a canned extraction sourced from the ACR-36 corpus, zero external API calls. Wired into compose, `.env.example`, CI, and a new unstubbed Playwright spec. | `daa9ad9` ([PR #54](https://github.com/jesgueva/acra_dev/pull/54)) |
| A10-3 | CI: Playwright job, all 20 Jest files, frontend coverage gate | **ACR-46** | ✅ done — Playwright and all 20 Jest files now run every PR with a coverage gate. Follow-up (#48) hardened CI container/port isolation. | `97a6555` ([PR #43](https://github.com/jesgueva/acra_dev/pull/43)) + [#48](https://github.com/jesgueva/acra_dev/pull/48) |
| A10-4 | Lockfile + Node/Python version reconciliation, pin AI SDKs | **ACR-42** | ✅ done — one Node (24) and Python (3.13) version repo-wide; `requirements.lock` pins 57 packages. Folded into A10-1. | `098cc2e` ([PR #37](https://github.com/jesgueva/acra_dev/pull/37)) |
| A10-5 | Privilege-parity test (seed vs migrations); fix/delete `create_admin.py` | **ACR-40** | ✅ done — deleted the broken `create_admin.py`; migration `016` grants `master_data.*` (same 403 defect ACR-35 fixed for `shipping.*`). New parity tests guard both grant directions. | `9a69bf0` ([PR #53](https://github.com/jesgueva/acra_dev/pull/53)) |
| A10-6 | LICENSE + data-provenance doc | **ACR-47** | ✅ done — added `LICENSE` (MIT) + `docs/DATA_PROVENANCE.md`, carving out `frontend/acra_logo.png`. | `308c46b` ([PR #46](https://github.com/jesgueva/acra_dev/pull/46)) |
| A10-7 | README troubleshooting, expected-output transcript, runbook | **ACR-51** | ✅ done — README gains a Troubleshooting table, hardware assumptions, and a captured bring-up transcript; new `docs/RUNBOOK.md` adds a packaging diagram. Docs-only. | `0fa3a59` ([PR #55](https://github.com/jesgueva/acra_dev/pull/55)) |
| A10-8 | Repo hygiene (untrack `backend/.coverage`, `plans/` policy) | **ACR-48** | ✅ done — untracked `backend/.coverage`; documented the `plans/` gitignore policy in CONTRIBUTING.md. | `bf36b55` ([PR #50](https://github.com/jesgueva/acra_dev/pull/50)) |
| **A10-writeup** | The A10 hardening package itself — audit, clean-environment pass, friction log | — | ✅ **evidence captured, 12 sections drafted**; pending walkthrough recording + compile. The pass found the documented test command (`docker compose run --rm backend pytest tests/`) exiting non-zero and running **nothing** — 5 tests reach outside the backend image and assert instead of skipping. **0 → 630 passed / 25 skipped.** Plus 3 README claims that *understated* the artifact (Shipments declared unreachable though KI-08 is resolved; a mock fallback denied in a doc that describes it twice; status pinned 2 hard stops back). 11 friction entries, 10 fixed, F-07 carried open. | `8490fd0` ([#58](https://github.com/jesgueva/acra_dev/pull/58)) · `db53afe` ([#59](https://github.com/jesgueva/acra_dev/pull/59)) · `8c769c0` ([#60](https://github.com/jesgueva/acra_dev/pull/60)) · [#61](https://github.com/jesgueva/acra_dev/pull/61) |
...
| **A8-7** | Subsystem diagram marking measurement points (ACR-49) | 2026-08-01 | `ea5dec5` — [PR #52](https://github.com/jesgueva/acra_dev/pull/52), merged from `ticket-49/measurement-points-diagram` | Plan at `plans/plan_ticket_A8-7.md`. `docs/architecture.md` gained a `## Measurement points (A8-7)` section: an ASCII diagram (same shape as the existing system-decomposition diagram) tagging each edge with the `validation-run.sh` stage(s) measured there, plus a stage → script → component(s) crossed → what it measures → ticket table covering all six stages (6a pipeline trace, 6b OCR round-trip / A8-4, 6c API latency / A8-2+A8-3, 6d concurrency ablation / A8-5, 6e OCR provider comparison / A8-4, 6f aggregation bench / A8-6). Cross-referenced from a `validation-run.sh` comment so the mapping is discoverable from either direction. Also names `app/core/benchmark.py` and `app/core/observability.py` as the two cross-cutting modules underlying most stages, closing the gap where the component map lists `core/` only as an undifferentiated bullet. Docs-only — no app code, migration, or test surface touched. |
| **A10-5** | Privilege-parity test; delete `create_admin.py` (ACR-40) | 2026-08-01 | `9a69bf0` — [PR #53](https://github.com/jesgueva/acra_dev/pull/53), merged from `ticket-49/privilege-parity-create-admin` | Deleted `backend/scripts/create_admin.py` — only 2 of its 8 granted privilege names matched a real privilege, and it defaulted to port 5432; `seed_fake_data.py` already creates the same admin correctly. Migration **016** grants `master_data.view`/`master_data.manage` to `company_admin` — the identical 403-on-every-role defect ACR-35 fixed once already for `shipping.*` (`routers/contacts.py`/`products.py` enforce these but no migration had ever granted them) — and revokes `inventory.alerts.manage`, dead weight granted since migration `002` and checked by nothing. Mirrors both into `seed_fake_data.py`'s `ROLE_DEFINITIONS`, plus adds the `inventory.reserve` grant migration `010` already made but the seed script never mirrored. **New tests close the class of bug for good, in both directions:** `tests/test_privilege_parity.py` (no DB — every router-enforced privilege is seeded; no seed grant goes unused) and `tests/integration/test_privilege_parity_migrations.py` (live DB — no migration grant goes unused either, reading the migrated database directly since migrations grant privileges through four different code shapes that only agree once applied). Retired the `master_data.*` entries from `test_shipping_privileges.py`'s `_UNSEEDED_KNOWN_GAPS` allowlist. pytest 682 passed / 17 skipped / 96%, run twice (locally-seeded and fresh migrations-only) to match CI exactly; migration `016` round-trip verified. |
| **A10-2** | OCR offline/mock mode + committed sample fixtures (ACR-50) | 2026-08-01 | `daa9ad9` — [PR #54](https://github.com/jesgueva/acra_dev/pull/54), merged from `ticket-50/ocr-mock-mode` | New `OCR_MOCK_MODE` config flag short-circuits `ocr_service.process_image_bytes()` to a canned extraction before either provider is called — the receiving/OCR flow is runnable end to end with zero external API dependency. Reuses the ACR-36 corpus fixture already committed for exactly this (`sample_bol_gridded.png`); the mock response mirrors that fixture's ground truth as a literal, not an import, per `ocr_bench/__init__.py`'s rule that the app must never import that package. Wired through `docker-compose.yml` (default `false`), `.env.example`, and the `e2e-playwright` CI job (`true`, so a new unstubbed spec runs with no secrets provisioned). No DB/migration changes — `OCRResponse` was never persisted. New Playwright spec uploads the real fixture against the real, unstubbed `/deliveries/ocr` endpoint — every other case in the file stubs the network boundary; this one deliberately doesn't, to prove the endpoint itself needs no API key. Gates: pytest 687 passed / 17 skipped / 96%, Jest 20/20 suites / 148, lint 0 errors, build OK, smoke PASSED, **Playwright 105/105** (including the new live-mock spec). |
| **A10-7** | README troubleshooting, expected-output transcript, runbook (ACR-51) | 2026-08-01 | `0fa3a59` — [PR #55](https://github.com/jesgueva/acra_dev/pull/55), merged from `ticket-51/readme-runbook` | Added a README `## Troubleshooting` table (8 symptom → cause → fix entries: port/name collisions, the `NEXT_PUBLIC_API_URL`/`BACKEND_URL` build-vs-runtime split, `migrate`'s expected health status, stale-volume schema mismatches, OCR skip-without-key, local `.env`/DB setup, Node version drift, the coverage-target gotcha) and a `## Hardware assumptions` subsection with real `docker stats`/`docker images` numbers from a clean build, not guessed. Added a captured expected-output transcript for `docker compose up -d --build` under Quickstart A. New `docs/RUNBOOK.md` — packaging diagram in the same ASCII style as `docs/architecture.md`, plus bring-up/seed/health-check/log/teardown commands and the multi-stack-port-override pattern. **Verification, not assertion:** every command shown was actually run — a clean `docker compose down -v && up -d --build` produced the captured transcript, and the container-name-collision symptom was deliberately reproduced and confirmed the documented override fixes it. Review-loop fix: two doc/reality mismatches caught — `migrate`'s healthcheck is disabled in `docker-compose.yml` so it never reports "unhealthy" (as the first draft claimed), and the frontend Dockerfile's real healthcheck targets `/en/login`, not `/` (which redirects under next-intl). Docs-only. Gates: pytest 682/17 skipped/96%, Jest 20/20, lint 0 errors, build OK, smoke + compose-smoke PASSED. |
