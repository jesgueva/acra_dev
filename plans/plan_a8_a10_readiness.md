...
**Date:** 2026-07-30
**§8 board last reconciled against Linear + GitHub:** 2026-07-31, `master` @ `8b84f9a`
(`ticket-36: A8-4 — OCR accuracy gate & provider comparison bench (#ACR-36) (#40)`), including a
pass over open worktrees for ACR-45 (`ticket-45/aggregation-benchmark`) and ACR-46
(`ticket-46/ci-playwright-jest-coverage`), both **In Progress** on Linear with uncommitted/unmerged
work — neither has a PR open.
...
| A8-5 | Comparative concurrency study (3-arm ablation) | **ACR-44** | ✅ **merged** — review follow-up open: [PR #39](https://github.com/jesgueva/acra_dev/pull/39) (two-directional oracle, contention witness, flake fix), not yet merged | [PR #38](https://github.com/jesgueva/acra_dev/pull/38) → `0547b38` |
| A8-6 | Aggregation benchmark at volume (RSK-04) | **ACR-45** | 🔄 **in progress, substantially built** — worktree `ticket-45/aggregation-benchmark`, 4 commits on top of `0547b38`: `aggregation_bench.py` d | — |
...
| A10-3 | CI: Playwright job, all 20 Jest files, frontend coverage gate | **ACR-46** | 🔄 **in progress** — §6 #3 resolved (plan: every PR, required check, per `plans/plan_ticket_46.md` §4); worktree `ticket-46/ci-playwright-jest-coverage` off `8b84f9a`, no commits yet but working-tree edits in  | **ACR-42** | ✅ **merged** — folded into A10-1 | [PR #37](https://github.com/jesgueva/acra_dev/pull/37) → `098cc2e` |
| A10-5 | Privilege-parity test (seed vs migrations); fix/delete `create_admin.py` | (ACR-40 adjacent) | ⬜ | — |
| A10-6 | LICENSE + data-provenance doc | **ACR-47** | ✅ **done, PR open** — `LICENSE` (MIT, code only) + `docs/DATA_PROVENANCE.md` explicitly carving out `frontend/acra_logo.png` | `ticket-47/license-data-provenance` |
