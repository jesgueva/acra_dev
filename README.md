...
| `docker compose ps` shows `migrate` as `unhealthy` | Expected, not a bug — `migrate` is a one-shot job (`alembic upgrade head`, then exits), but it inherits the backend image's healthcheck, which curls a server this job never starts | Ignore; check its actual outcome with `docker compose logs migrate` — it should show `Exited (0)` |
