...
``product_id`` is the "item" and ``state`` reuses the ``LotStatus`` vocabulary from
``app.models.stock_movement``, whose values match the ``inventory_lots.status`` check constraint.
"""
