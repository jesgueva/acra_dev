...
class WorksheetLineCreate(BaseModel):
    product_id: int
    planned_quantity: int = Field(..., gt=0)  # integer ×100
...
class WorksheetCloseLine(BaseModel):
    line_id: int
    actual_quantity: int = Field(..., ge=0)  # ×100; 0 is legal (line produced nothing)


class WorksheetCloseRequest(BaseModel):
    """``expected_version`` is the optimistic guard the client read from the worksheet.

    A stale value means another close already won the race, and the request is rejected with 409
    rather than silently consuming stock a second time.
    """
...
class WorksheetLineResponse(BaseModel):
    ...
    actual_quantity: Optional[int] = None  # ×100, populated at close
