...
def _create_session(lot, products=()):
    """Session for `create_shipment` called directly.

    Query order: the lots, then the delivery note's document-number de-duplication (nothing taken),
    then the products. No contact query runs — these bodies carry neither a client nor a carrier,
    and `contact_service.load_by_ids` short-circuits on an empty set.
    """
    def _rows(rows):
    ...
    return _make_service_session(
        [_rows([lot]), _rows([]), _rows(products)],
        assign_ids=True,
        created_at=_CREATED_AT,
    )
