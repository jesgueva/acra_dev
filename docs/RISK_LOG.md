...
Last reviewed: **2026-08-01** (Hard Stop 4 — Midpoint Technical Evidence Review) · Owner role key: *Lead Dev* (single-developer applied project).
...
| RSK-03 | AI receiving-document extractor regresses below the established OCR accuracy baseline, or the hosted provider is slow/unstable. | Med | Med | Schema-constrained output + provider fallback chain (primary → fallback); no-regression accuracy gate and a latency gate in the test plan; manual entry always available. **Measured (ACR-36 / A8-4, 2026-07-31):** 7-layout labelled corpus × 3 rounds, both providers called directly — gemini-2.5-flash header 96.25% / item-F1 99.25%, claude-sonnet-4-6 header 96.43% / item-F1 97.1%; recorded with full provenance in `backend/tests/fixtures/ocr/baseline.json` and enforced by `tests/integration/test_ocr_accuracy.py` (`OCR_BENCH_LIVE=1`) with a 0.05 tolerance and a two-directional negative control. **The "established baseline" this risk names did not exist and could not have:** SC-2 defines it as the *v6* pipeline's accuracy, A4's ADR-05 and TC-03 describe that pipeline as "v6 Tesseract + regex, kept runnable as the comparator" — but v6 never shipped Tesseract (it shipped Gemini 2.5 Flash → Claude Sonnet 4.6, the *same pair* running today), and no v6 accuracy figure was ever captured; the W1/W2 capture SC-2 defers to never happened. The comparator was therefore both unavailable and self-referential. **SC-2 is restated against the self-recorded ACR-36 baseline** as a no-regression gate plus a live provider-vs-provider comparison — see A8 §06/§08. Reproduce: `ACRA_BENCH_PROFILE=publication ./scripts/validation-run.sh` (stage 6e), or `cd backend && python -m scripts.ocr_bench.run_bench --provider both --repeat 3 --delay 13`. Stays *Mitigating*: the corpus is synthetic and therefore cleaner than a real phone photo, so the figures are an **upper bound** and real-BOL validation is still open. | Lead Dev | Mitigating | 2026-06-16 | 2026-08-01 |
| RSK-04 | Ledger on-hand/reserved/available aggregation is slow at data volume. | Med | Low | Index by `(item, state)`; keep a periodic-snapshot fallback in reserve if reads degrade. **Measured (ACR-45 / A8-6, 2026-07-31):** the mitigation was only ever half-applied — `stock_reservations` was indexed in revision 010, `inventory_lots` had no index beyond its primary key, so the `_on_hand` half of every `availability` call sequentially scanned the table. Migration `015` adds `(product_id, status) INCLUDE (quantity_on_hand)`. Server-side execution time for that aggregate, via real service calls at 1k/10k/50k/200k bench lots (over ~14 800 ambient seeded lots): **14.134 ms → 0.029 ms at 200 000 lots (487×)**, plan `Seq Scan` → `Index Only Scan` at every volume. The decisive shape: unindexed cost grows with the table (2.9 → 2.6 → 3.9 → 14.1 ms) while indexed cost stays **flat and sub-0.04 ms** (0.034 → 0.038 → 0.009 → 0.029 ms). **The periodic-snapshot fallback is retired** — a flat sub-0.04 ms aggregate out to 200k lots has no read-degradation curve to defend against, and a snapshot would add a staleness window plus an invalidation path for no measured gain. Reproduce: `ACRA_BENCH_PROFILE=publication ./scripts/validation-run.sh` (stage 6f) → `aggregation-bench-summary.json` + `aggregation-explain-plans.txt`. (`validation-evidence/` is git-ignored generated output, so this cites the command rather than a path that resolves for nobody; the captured run is archived with the A8 package.) Guarded by `tests/integration/test_aggregation_at_volume.py`, whose negative control drops the index inside a rolled-back transaction and asserts the plan degrades. **The other three measured paths, reported rather than hidden:** `list_alerts` groups over the whole table with no `WHERE` and **stays a `Seq Scan` at every volume** (45.6 → 29.7 ms at 200k — the covering index shaves heap reads off the `GROUP BY` but cannot change the plan); `list_inventory` was already index-only via the primary key (~1.0×); `export_csv` is unpaginated and is carried separately as RSK-10. | Lead Dev | **Resolved** | 2026-06-16 | 2026-07-31 |
...
| ISS-05 | ~~OCR line-item extraction is layout-sensitive (1/3 rows on a cramped table, 3/3 on a gridded one)~~ — **superseded by measurement (ACR-36 / A8-4, 2026-07-31).** Across the 7-layout corpus `borderless_cramped` scores the same as `gridded` on both providers: **table layout alone does not degrade extraction.** What does is **image quality** — the `degraded_fax` layout produces genuine misreads on both providers (a BOL reference read `MS-2026-0991` for `MS-2026-0891`; an item name hallucinated as "Placa Inoxidizable" for "Placa Antideslizante"). The original single-document observation was a sample of one. | Low | Restated, not closed: the corpus is synthetic, so the *original* claim may still hold on real client documents. Real-BOL validation stays open under RSK-03. Detail in KI-09. | Lead Dev | Monitoring | 2026-06-23 | 2026-08-01 |
| ISS-06 | Three divergent stock-drawdown implementations. ACR-30's negative control demonstrates in CI that the unguarded read-modify-write shape loses updates — and that is the shape `inventory_service.adjust_quantity` and `shipment_service.create_shipment` still use, while `allocation_service` uses the SERIALIZABLE approach ADR-02 rejects. Only the worksheet close is guarded. **Quantified (ACR-44 / A8-5, 2026-07-31):** the four shapes measured on one fixed workload, 2→32 concurrent closers — the unguarded shape loses **281 updates across the sweep** and completes **3%** of attempts at 32, while ADR-02's protocol is the only arm holding **0 lost updates and 100% success at every level**, and the only one whose goodput *rises* rather than collapses as concurrency climbs. Bare SERIALIZABLE loses nothing but also completes ~3% at 32; adding 5 bounded retries lifts that only to roughly a fifth to a quarter of the work. **Reproduced three times** — the lost-update count came back 281 / 280 / 281 across independent runs, while absolute goodput varied ~2× with machine load, so this entry quotes the correctness columns and the direction of the curves rather than throughput figures that do not transfer between hosts. So the gap is not stylistic — two live code paths use the arm the study measured losing data. | Med | **Now the highest-value next step**, ahead of new surfaces: apply the ADR-02 protocol to the remaining drawdown paths, or converge them on one helper when the ledger lands (ACR-31). Reproduce: `ACRA_BENCH_PROFILE=publication ./scripts/validation-run.sh` (stage 6d). | Lead Dev | Open | 2026-07-23 | 2026-08-01 |
...
- **2026-08-01** — Hard Stop 4 (Midpoint Technical Evidence Review) register reconciliation. No code
  behaviour changed; this pass makes the register say what the measurements found.
  - **RSK-03** carries the measured accuracy figures and records a **correction to SC-2 itself**: the
    "v6 baseline" the criterion compares against was never captured, and the comparator A4's ADR-05
    and TC-03 name — "v6 Tesseract + regex" — never existed, because v6 shipped the same
    Gemini→Claude pair running today. SC-2 is restated against the self-recorded ACR-36 baseline.
    Tesseract is not being reintroduced; the design decision to run vision-LLM extraction stands.
  - **ISS-05** restated: the layout-sensitivity claim was a sample of one and does not survive the
    7-layout corpus. Image quality, not table layout, is what degrades extraction.
  - **ISS-06** quantified with the A8-5 ablation, which promotes it from a consistency complaint to
    the register's highest-value open item — two live paths use the arm measured losing 281 updates.
  - **RSK-04**'s evidence pointer changed from a `validation-evidence/` path (git-ignored, so it
    resolved for no reader) to the command that regenerates it. Same for the new RSK-03 pointer.
  - Register had been carrying "Last reviewed: 2026-06-23" while holding 2026-07-23 and 2026-07-31
    content; re-dated.
